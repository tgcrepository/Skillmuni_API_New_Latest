    ...
    public class getUserScoreController : ApiController
    {
        public HttpResponseMessage Get(int UID,int OID)
        {
            ...
            using (m2ostnextserviceDbContext db = new m2ostnextserviceDbContext())
            {
                    ...
                    scorelog = db.Database.SqlQuery<tbl_user_game_score_log>("select * from tbl_user_game_score_log where id_game={0} and id_user={1} ", GameId, UID).ToList();
                    double score = 0;
                    ...
                    splscorelog = db.Database.SqlQuery<tbl_user_game_special_metric_score_log>("select * from tbl_user_game_special_metric_score_log where id_game={0} and id_user={1}", GameId, UID).ToList();
                    double splscore = 0;
