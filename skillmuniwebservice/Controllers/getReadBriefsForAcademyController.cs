...
namespace m2ostnextservice.Controllers
{
    public class getReadBriefsForAcademyController : ApiController
    {
        ...
        public HttpResponseMessage Get(int UID, int OID, int AcadamyTileId)//string ENC
        {
            ...
            List<BriefAPIResource> readlist = new List<BriefAPIResource>();
            if (list != null)
            {



                    readlist = list.Where<BriefAPIResource>(t => t.read_status == 1).ToList();

                return Request.CreateResponse(HttpStatusCode.OK, readlist);
            }
            else
            {
                return Request.CreateResponse(HttpStatusCode.NoContent, readlist);
            }
